	...
	void run() {
		...
		case END:
			...
			if (sampleRoll > 50) { // adjust great success ot 50% and above
				...
				result = 1;
			} else if ( (40 + rollMod) < sampleRoll) { // failure your roll < 50%
				result = 2;
			} else { // success
			...
			switch(result) {
				...
				case 4:
					success = true;
					death = true; //changed to agro instead of killing, because we allow the sampling of very high level creatures
					break;
				case 5:
					success = true;
					break;
