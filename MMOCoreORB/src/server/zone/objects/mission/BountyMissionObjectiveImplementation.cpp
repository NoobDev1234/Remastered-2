...
void BountyMissionObjectiveImplementation::handlePlayerKilled(ManagedObject* arg1) {
	...
	if (owner != NULL && killer != NULL && !completedMission) {
		...
		String bhLastName = owner->getLastName();
		...
		if (owner->getObjectID() == killer->getObjectID()) {
			...
			if (zoneServer != NULL) {
				...
				if (target != NULL) {
					...
					trophy = (owner->getZoneServer()->createObject(STRING_HASHCODE("object/tangible/loot/creature/loot/collections/broken_lightsaber_hilt_001.iff"), 2)).castTo<TangibleObject*
				...
				if (trophy != NULL) {
					Locker locker(trophy);
						...
						if (inventory->transferObject(trophy, -1, true)) {
							...
							abort();
							owner->sendSystemMessage("Your inventory is full, could not create Trophy");
			...
			ZoneServer* zoneServer = owner->getZoneServer();
			if (zoneServer != NULL) {
				ManagedReference<CreatureObject*> jedi = zoneServer->getObject(mission->getTargetObjectId()).castTo<CreatureObject*>();
				ManagedReference<SceneObject*> inventory = jedi->getSlottedObject("inventory");

				StringBuffer trophyBuffer;
				trophy = NULL;

				trophy = (killer->getZoneServer()->createObject(STRING_HASHCODE("object/tangible/loot/creature/loot/collections/broken_mando_helmet.iff"), 2)).castTo<TangibleObject*>();

				//object/tangible/collection/shared_rare_pistol_deathrain.iff
				//object/tangible/collection/shared_rare_carbine_darklighter_e11.iff
				...
				if (trophy != NULL) {
					Locker locker(trophy);
						...
						if (inventory->transferObject(trophy, -1, true)) {
							...
							if (bhLastName != "")
								trophyBuffer << bhName << " " << bhLastName << "'s Helmet";
							else
								trophyBuffer << bhName << "'s Helmet";

							trophy->setCustomObjectName(trophyBuffer.toString(), false);

							trophy->sendTo(jedi, trophy);
						} else {
							trophy->destroyObjectFromDatabase(trophy);
							abort();
							jedi->sendSystemMessage("Your inventory is full, could not create Trophy");
