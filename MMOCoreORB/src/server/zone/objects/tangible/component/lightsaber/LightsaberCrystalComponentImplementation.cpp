...
void LightsaberCrystalComponentImplementation::generateCrystalStats() {
	...
	int minHit = crystalData->getMinHitpoints();
	int maxHit = crystalData->getMaxHitpoints();
	int crystalCondition = getRandomizedStat(minHit, maxHit, itemLevel);

	crystalCondition = int(hardCapIntStatGeneration(minHit, maxHit, crystalCondition));
...
void LightsaberCrystalComponentImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	//if (colorMax != 31) {
		int finalColor = Math::min(color, 41);
		setColor(finalColor);
