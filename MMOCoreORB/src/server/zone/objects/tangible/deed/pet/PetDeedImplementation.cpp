...
void PetDeedImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	ManagedReference<CreatureTemplate*> petTemplate =  creatureTemplateManager->getTemplate( mobileTemplate.hashCode() );
	if (petTemplate != NULL) {
		//info("Calculated level: " +  String::valueOf(level));
		//info("Pet Template level: " +  String::valueOf(petTemplate->getLevel()));
...
int PetDeedImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
	...
	if (selectedID == 20) {
		...
		/*
			player->sendSystemMessage("level :" + String::valueOf(level));
			player->sendSystemMessage("isVicious :" + String::valueOf(isVicious));
			player->sendSystemMessage("maxLevelofPets :" + String::valueOf(player->getSkillMod("tame_level")));
			player->sendSystemMessage("tame_aggro :" + String::valueOf(player->getSkillMod("tame_aggro")));

			info( "Level :" + String::valueOf(level) );
			info( "Creature isVicious :" + String::valueOf(isVicious) );
			info( "Max Level Of Pets :" + String::valueOf(maxLevelofPets) );
			info( "Tame Aggro :" + String::valueOf(player->getSkillMod("tame_aggro")) );
		*/
...
void PetDeedImplementation::adjustPetLevel(CreatureObject* player, CreatureObject* pet) {
	...
	//player->sendSystemMessage("Calculated Creature Level: " + String::valueOf(newLevel));
	//info("Calculated Creature Level: " + String::valueOf(newLevel));
