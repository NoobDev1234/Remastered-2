...
void ResourceContainerImplementation::setQuantity(uint32 quantity, bool doNotify, bool ignoreMax) {
	...
	if (newStackSize > 0) {
		if (parent != NULL) {
			...
			harvestedResource->setCustomObjectName(spawnObject->getName() + "-" + spawnObject->getFinalClass(), true);
...
void ResourceContainerImplementation::split(int newStackSize) {
	...
	if(sceneParent->transferObject(newResource, -1, true)) {
		newResource->setCustomObjectName(spawnObject->getName() + "-" + spawnObject->getFinalClass(), true);
