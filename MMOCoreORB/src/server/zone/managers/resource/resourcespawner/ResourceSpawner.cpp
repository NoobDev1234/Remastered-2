...
ResourceSpawner::ResourceSpawner(ManagedReference<ZoneServer*> serv, ZoneProcessServer* impl) {
...
bool ResourceSpawner::addResourceToPlayerInventory(CreatureObject* player, ResourceSpawn* resourceSpawn, int unitsExtracted) {
	...
	//error("family name: " + resourceSpawn->getFamilyName() + " Final Class: " + resourceSpawn->getFinalClass() + " MissionSpawnFamily:" + resourceSpawn->getSurveyMissionSpawnFamilyName());
	harvestedResource->setCustomObjectName(resourceSpawn->getName() + "-" + resourceSpawn->getFinalClass(), true);
