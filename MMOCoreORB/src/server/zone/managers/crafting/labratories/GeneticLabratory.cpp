...
void GeneticLabratory::setInitialCraftingValues(TangibleObject* prototype, ManufactureSchematic* manufactureSchematic, int assemblySuccess) {
	...
	for (int i = 0; i < craftingValues->getExperimentalPropertySubtitleSize(); i++) {
		...
		//info("MaxValue: " + String::valueOf(maxValue));
		//info("Setting Max Percentage to: " + String::valueOf(maxValue/1300.0f));

		// determine max percentagerr
		craftingValues->setMaxPercentage(title, maxValue/1300.0f);
